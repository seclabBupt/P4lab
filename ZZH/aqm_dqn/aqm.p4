...
typedef bit<32> queue_length;
typedef bit<32> queue_delay;
typedef bit<32> time_cha;
...
struct metadata {
    ...
    bit<32>  port_timestamp;
    bit<32>  port_timestampcha;
}
...
register<bit<32>>(64) entimestamp_register;
...
control MyEgress(inout headers hdr,
    ...
    action update_timestamp(){
        entimestamp_register.read(meta.port_timestamp,(bit<32>)standard_metadata.egress_port);
        meta.port_timestampcha=(bit<32>)(standard_metadata.enq_timestamp-meta.port_timestamp);
        entimestamp_register.write((bit<32>)standard_metadata.egress_port,standard_metadata.enq_timestamp);
    }
    ...
    action h_drop(bit<32> C,bit<32> k){
        if(100*((bit<32>)standard_metadata.deq_qdepth)>=k*C){
            meta.mark_drop = 1;
        }
    }
    ...
    apply {
        ...
        if(hdr.ipv4.protocol==251){
            hdr.jiance.queuelength=(bit<32>)standard_metadata.enq_qdepth;
            hdr.jiance.queuedelay=(bit<32>)standard_metadata.deq_timedelta;
            hdr.jiance.timecha=(bit<32>)meta.port_timestampcha;
            meta.mark_drop=0;
        }
        if(meta.port_sum<=1000&&exist.apply().hit){
            heavy_drop.apply();
        }
        else if(hdr.ipv4.srcAddr==167774977){
            meta.mark_drop=0;
        }
        else{
            aqm.apply();
