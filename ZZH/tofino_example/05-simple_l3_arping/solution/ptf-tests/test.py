...
"""
Simple PTF test for simple_l3_arping
"""
...
class HostL3Switch(BaseProgramTest):
    def runTest(self):
        ...
        self.ipv4_host.entry_add(self.dev_tgt, key_list, data_list);
        print("  Added an entry to ipv4_host: {} --> l3_switch({}, {}, {})".
              format(ipv4_dst, egress_port, new_mac_da, new_mac_sa))
...
class TestGroup1(BaseProgramTest):
    ...
    def setUp(self):
        ...
        self.programTable(self.ipv4_host, [
            ([("meta.dst_ipv4", "192.168.1.1")],
             "Ingress.set_nexthop", [("nexthop", 101)]),
            ([("meta.dst_ipv4", "192.168.1.2")],
             "Ingress.set_nexthop", [("nexthop", 102)]),
            ([("meta.dst_ipv4", "192.168.1.3")],
             "Ingress.set_nexthop", [("nexthop", 1)]),
            ([("meta.dst_ipv4", "192.168.1.254")],
        ...
        self.programTable(self.ipv4_lpm, [
            ([("meta.dst_ipv4", "192.168.1.0", None, 24)],
             "Ingress.set_nexthop", [("nexthop",    0)]),
            ([("meta.dst_ipv4", "192.168.3.0", None, 24)],
              "Ingress.set_nexthop", [("nexthop", 101)]),
            ([("meta.dst_ipv4", "192.168.5.0", None, 24)],
              "Ingress.set_nexthop", [("nexthop", 101)]),
            ([("meta.dst_ipv4", "192.168.7.0", None, 24)],
              "Ingress.set_nexthop", [("nexthop", 101)]),
           ([("meta.dst_ipv4", "192.168.0.0",  None, 16)],
              "Ingress.set_nexthop", [("nexthop",   1)]),
          ([("meta.dst_ipv4", "0.0.0.0",       None,  0)],
              "Ingress.set_nexthop", [("nexthop",   0)]),
        ])
...
class ArpWhois1(TestGroup1):
    def runTest(self):
        # Test Parameters. See TestGroup1.setUp for details
        ingress_port = 0
        ipv4_dst     = "192.168.3.7"
        arp_mac      = "00:00:01:00:00:01"
        ...
        #
        # Prepare the test input and output packets
        #
        ...
        print("Sending Who-has {} into port {}".
              format(ipv4_dst, ingress_port))
        send_packet(self, ingress_port, send_pkt)
...
class IcmpTest1(TestGroup1):
    def runTest(self):
        ...
        # Prepare the test input and output packets
        #
        send_pkt = simple_icmp_packet(ip_dst=ipv4_dst, pktlen=100)

        exp_pkt = copy.deepcopy(send_pkt)
        exp_pkt[Ether].dst = send_pkt[Ether].src
        exp_pkt[Ether].src = send_pkt[Ether].dst
        exp_pkt[IP].src    = send_pkt[IP].dst
        exp_pkt[IP].dst    = send_pkt[IP].src
        exp_pkt[IP].ttl    = send_pkt[IP].ttl - 1
        exp_pkt[ICMP].type = 0
        exp_pkt[ICMP].chksum = 0

        print("Sending ICMP Echo Request for {} into port {}".
              format(ipv4_dst, ingress_port))
        send_packet(self, ingress_port, send_pkt)

        print("Expecting ICMP Echo Reply from {} at port {}".
              format(ipv4_dst, ingress_port))
        verify_packet(self, exp_pkt, ingress_port)
